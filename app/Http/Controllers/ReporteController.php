...
class ReporteController extends Controller
{
    ...
    public function getReporteCuadreCaja(Request $request)
    {
        ...
        $facturasManualesExoneradas = Factura::with('cobros', 'modulo')
            ->whereBetween('fecha', array($annoDesde . '-' . $mesDesde . '-' . $diaDesde,  $annoHasta . '-' . $mesHasta . '-' . $diaHasta))
            ->whereHas('modulo', function ($q) {
                $q->where('nombre', 'EXONERADAS');
            })
        ...
        $facturas = \App\Factura::with('cobros', 'modulo')
            ->whereBetween('fecha', array($annoDesde . '-' . $mesDesde . '-' . $diaDesde,  $annoHasta . '-' . $mesHasta . '-' . $diaHasta))
            ->where('facturas.deleted_at', null)
            ->whereHas('modulo', function ($q) {
                $q->where('nombre', '<>', 'EXONERADAS');
        ...
        $facturasExoneradas = \App\Factura::with('cobros', 'modulo')
            ->whereBetween('fecha', array($annoDesde . '-' . $mesDesde . '-' . $diaDesde,  $annoHasta . '-' . $mesHasta . '-' . $diaHasta))
            ->where('facturas.deleted_at', null)
            ->whereHas('modulo', function ($q) {
                $q->where('nombre', 'EXONERADAS');
        ...
        $facturasExoneradasTotal         = $facturasExoneradas->sum('total');
        ...
        $facturasManualesExoneradasTotal         = $facturasManualesExoneradas->sum('total');
        ...
        return view('reportes.reporteCuadreCaja', compact(
            'diaDesde',
            'mesDesde',
            'annoDesde',
            'diaHasta',
            ...
            'facturasCredito',
            'facturasAnuladas',
            'facturasAnuladasTotal',
            'tasasVendidas',
            'totalTasas',
            'tipoTasas',
            'facturasManuales',
            'facturasManualesTotal',
            'facturasManualesAnuladas',
            'facturasManualesAnuladasTotal',
            'facturasManualesExoneradas',
            'facturasManualesExoneradasTotal',
            'facturasExoneradas',
            'facturasExoneradasTotal'
        ));
    }
